...
def plot_nmf_factor_2Dhistogram(adata, f1, f2, color_column, 
                               filter_column=None, filter_value=None,
                               use_log=False, cap_density_value=None):
    """
    Plot 2D histogram of NMF factor usage, split by a categorical variable.
    
    Parameters:
    -----------
    adata : AnnData
        Annotated data matrix with NMF factors in .obsm['X_nmf']
    f1 : int
        First NMF factor index to plot
    f2 : int
        Second NMF factor index to plot
    color_column : str
        Column in adata.obs to use for splitting plots
    filter_column : str, optional
        Column in adata.obs to use for filtering data
    filter_value : str, optional
        Value in filter_column to keep
    use_log : bool, default=False
        Whether to log-transform the factor values
    cap_density_value : float, optional
        Maximum value to cap the density colormap
    ...
    if filter_column is not None and filter_value is not None:
        mask = adata.obs[filter_column] == filter_value
        adata = adata[mask].copy()
    
    # Get factor values
    x = adata.obsm['X_nmf'][:, f1]
    y = adata.obsm['X_nmf'][:, f2]
    
    if use_log:
        x = np.log1p(x)
        y = np.log1p(y)
    
    # Get unique categories and create subplot grid
    categories = adata.obs[color_column].unique()
    n_cats = len(categories)
    
    # Create figure
    fig = plt.figure(figsize=(30, 7.5))
    gs = gridspec.GridSpec(2, n_cats)
    ...
    for i, cat in enumerate(sorted(categories)):
        ...
        h = ax.hist2d(x[mask], y[mask], bins=50, density=True)
        
        if cap_density_value is not None:
            h[3].set_clim(0, cap_density_value)
        
        ax.set_title(f'{color_column}={cat}')
