...
# THIS VERSION WAS ARCHIVED ON 2025-01-31
# IT ASSUMES THAT THE LOG-NORMAL DISTRIBUTION VARIANCE DOES NOT DEPEND ON COLONY SIZE
...
class MCMCInference:
    ...
    def compute_log_likelihood(self, params, p0, a, sigma, threshold):
        ...
        for t in self.times:
            ...
            lognormal_logprobs = -((pt.log(x_above) - mu)**2)/(2*sigma**2) - pt.log(x_above) - pt.log(sigma) - 0.5*pt.log(2*np.pi)
    ...
    def setup_model(self):
        ...
        with pm.Model() as self.model:
            ...
            a = pm.Normal('a', mu=a0, sigma=a0/2)  # Location parameter for log-normal
    ...
    def plot_predictions(self, figsize=(10, 5)):
        ...
        for i, t in enumerate(self.times):
            ...
            empirical_sems.append(np.std(x_t)/np.sqrt(len(x_t)))
            ...
            for n, p_n in zip(n_values[1:], px[1:]):
                y_plot += p_n * np.exp(-(np.log(x_plot) - 
                                       (np.log(n) + est_params['a']))**2 / 
                                     (2*est_params['sigma']**2)) / (est_params['sigma']*np.sqrt(2*np.pi))
