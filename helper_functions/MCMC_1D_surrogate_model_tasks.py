...
class ProbabilityDistributionDataset(Dataset):
    """Dataset class for probability distribution data with symmetry"""
    def __init__(self, init0_df: pd.DataFrame, init1_df: pd.DataFrame):
        """
        Parameters
        ----------
        init0_df : pd.DataFrame
            DataFrame for P0 distribution
        init1_df : pd.DataFrame
            DataFrame for P1 distribution
        """
        ...
        # Transform init1_df using symmetry
        params1 = np.log10(init1_df.iloc[:, :2].values)
        params1_swapped = params1[:, [1, 0]]  # swap q01 and q10
        dist1 = init1_df.iloc[:, 2:].values[:, ::-1]  # reverse the distributions
        ...
        self.linear_parameters = np.vstack([
            init0_df.iloc[:, :2].values,
            init1_df.iloc[:, [1, 0]].values  # swapped columns for init1
        ])
...
class SurrogateModel:
    ...
    def train(self, init0_df: pd.DataFrame, init1_df: pd.DataFrame, verbose: bool = True) -> Dict[str, List[float]]:
        """
        ...
        ----------
        init0_df : pd.DataFrame
            Training data for initial state 0
        init1_df : pd.DataFrame
            Training data for initial state 1
    ...
    def predict(self, parameters: np.ndarray, q0: float = 0.5) -> np.ndarray:
        """
        Predict probability distributions for given parameters and q0
        
        Parameters
        ----------
        parameters : np.ndarray
            Array of shape (n_samples, 2) containing parameter pairs (q01, q10)
        q0 : float
        ...
        with torch.no_grad():
            ...
            # Get P1 prediction with swapped parameters
            log_params_swapped = log_params[:, [1, 0]]  # swap q01 and q10
            P1_swapped = self.model(log_params_swapped)
            
            # Apply symmetry: P1(f) = P0(1-f) with swapped parameters
    ...
    def predict_df(self, parameters: np.ndarray = None, q0: float = 0.5,
                  n_grid: int = None, q_min: float = None, q_max: float = None) -> pd.DataFrame:
        """
        ...
        # Get predictions with specified q0
    ...
    def plot_model_predictions(self, df: pd.DataFrame, n_interpolations: int = 10, q0=0.5, figsize: Tuple[int, int] = (10, 6)):
        """
        ...
        predictions = self.predict(interpolated_params, q0=q0)
    ...
    def plot_random_prediction_with_neighbors(self, df: pd.DataFrame, n_samples: int = 1, figsize: Tuple[int, int] = (10, 6)):
        ...
        for idx, (ax, params) in enumerate(zip(axes, random_params)):
            ...
            pred_dist = self.predict(params.reshape(1, -1), q0=0.0)[0]
    ...
    def save(self, filepath: str):
        ...
        # Save other attributes (excluding the pytorch model)
        save_dict = {
    ...
    def load(cls, filepath: str):
        ...
        # Create dummy dataset to store column information
        model.dataset = type('DummyDataset', (), {
...
def save_model_dict(model_dict: Dict[int, 'SurrogateModel'], base_path: str):
    """
    Save a dictionary of models to disk
...
def load_model_dict(base_path: str) -> Dict[int, 'SurrogateModel']:
    """
    Load a dictionary of models from disk
