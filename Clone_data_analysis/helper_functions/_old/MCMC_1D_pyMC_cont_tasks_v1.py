# THIS VERSION WAS (also) ARCHIVED ON 2025-01-31
# IT ASSUMES THAT THE LOG-NORMAL DISTRIBUTION VARIANCE
# WITH MEAN VARIANCE THAT FOLLOWS 1/N REDUCTION WITH NUMBER 
# OF CELLS (CENTRAL LIMIT)
...
class MCMCInference:
    ...
    def compute_log_likelihood(self, params, p0, a, sigma, threshold):
        ...
        for t in self.times:
            ...
            s2_nt = pt.log(1+(pt.exp(sigma**2)-1)/(n_nonzero.reshape((-1, 1))* pt.pow(2,t)))
            mu_nt = (pt.log(n_nonzero).reshape((-1, 1)) + a + (sigma**2-s2_nt)/2)
    ...
    def setup_model(self):
        ...
        with pm.Model() as self.model:
            ...
            log_S = pm.Uniform('log_S', lower=np.log(1e-3), upper=np.log(1e1), dtype="float64")
            log_r = pm.Uniform('log_r', lower=np.log(1e-2), upper=np.log(1e2), dtype="float64")
            S = pm.Deterministic('S', pm.math.exp(log_S))
            r = pm.Deterministic('r', pm.math.exp(log_r))
            
            p0 = pm.Uniform('p0', lower=0, upper=1, dtype="float64")
            threshold = pm.Uniform('threshold', lower=5e-4, upper=5e-3, dtype="float64")


            # New parameters for modeling full NMF scores
            a0, sigma0 = self.estimate_lognormal_params()
            a = pm.Normal('a', mu=a0-np.log(3), sigma=sigma0)  # Location parameter for log-normal
            sigma = pm.HalfNormal('sigma', sigma=sigma0/2)  # Scale parameter for log-normal
    ...
    def run_inference(self, draws, tune, chains, cores, show_progressbar=True, compute_convergence_checks=True):
        ...
        with self.model:
            ...
            step = pm.CompoundStep([
                ...
                pm.Metropolis(vars=[self.model.a], scaling=1.0, tune_interval=100),
                pm.Metropolis(vars=[self.model.sigma], scaling=1.0, tune_interval=100)
            ])
    ...
    def create_mock_data(self, 
        ...
        for t in times:
            ...
            if np.any(nonzero_mask):
                ...
                s_nt = np.sqrt(np.log(1 + (np.exp(est_params['sigma']**2)-1)/(n_values[:, None]*2**t)))
                mu_nt = np.log(n_values[:, None]) + est_params['a'] + (est_params['sigma']**2 - s_nt**2) / 2
    ...
    def plot_predictions(self, figsize=(10, 5)):
        ...
        for i, t in enumerate(self.times):
            ...
            for n, p_n in zip(n_values, px):
                if n == 0:
                    ...
                    mu_nt = np.log(n) + est_params['a'] + (est_params['sigma']**2 - s_nt**2) / 2
            ...
            scaling_factor = np.max(hist_vals) / np.max(y_plot[x_plot > est_params['threshold']])
        ...
        # Plot expected number of clones
        ax = axs[1, 3]  # Second row
        ax.plot(self.times, predicted_E_pn, 'k-', label='Model')
        ax.set_xlabel('Time')
        ax.set_ylabel('Expected number of clones')
        ax.legend()
